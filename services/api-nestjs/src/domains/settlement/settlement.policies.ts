...
export class SettlementPolicies {
  ...
  static canAccessSellerSettlements(
    sellerId: string,
    userId?: string,
    userRole?: UserRole,
  ): boolean {
  ...
  static canUpdateSellerAccountStatus(
    sellerAccount: any,
    userId?: string,
    userRole?: UserRole,
  ): boolean {
  ...
  static canViewSellerAccountDetails(
    sellerAccount: any,
    userId?: string,
    userRole?: UserRole,
  ): boolean {
  ...
  static canUpdateSettlementSchedule(
    sellerId: string,
    userId?: string,
    userRole?: UserRole,
  ): boolean {
  ...
  static canViewSettlementSchedule(
    sellerId: string,
    userId?: string,
    userRole?: UserRole,
  ): boolean {
  ...
  static canViewFinancialReports(
    userId?: string,
    userRole?: UserRole,
  ): boolean {
  ...
  static canProcessSettlementAmount(
    amount: number,
    sellerTier: string,
  ): boolean {
  ...
  static canProcessManualSettlement(
    userId?: string,
    userRole?: UserRole,
  ): boolean {
  ...
  static requiresApproval(settlement: any, sellerAccount: any): boolean {
    ...
    if (settlement.netAmount > 5000000) {
      // ₹50,000
      return true;
    }
  ...
  static requiresAdditionalVerification(
    ...
    if (settlement.netAmount > 10000000) {
      // ₹1,00,000
      return true;
    }

    // Settlements to new bank accounts require verification
    if (sellerAccount.bankAccountAge < 30) {
      // Bank account added less than 30 days ago
      return true;
    }

    // Unusual settlement patterns require verification
    if (settlement.transactionCount < 5 && settlement.netAmount > 1000000) {
      // Few transactions but high amount
      return true;
    }
  ...
  static isWithinRiskLimits(settlement: any, sellerAccount: any): boolean {
    ...
    const maxSettlementsPerDay = this.getMaxSettlementsPerDay(
      sellerAccount.tier,
    );
  ...
  static requiresComplianceCheck(settlement: any, sellerAccount: any): boolean {
    ...
    if (settlement.netAmount > 20000000) {
      // ₹2,00,000
      return true;
    }
    ...
    if (
      sellerAccount.businessDetails?.category &&
