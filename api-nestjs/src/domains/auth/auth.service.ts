...
export class AuthService {
  ...
  async googleAuth(googleAuthDto: GoogleAuthDto): Promise<AuthResponseDto> {
    ...
    // In production, verify the token with Google OAuth2 API
    // For now, this expects the frontend to send a verified Google ID token
    // The frontend should use Google Sign-In library to get the token
    ...
    try {
      ...
      // For MVP: Extract email from token (frontend should send verified data)
      // This is NOT secure for production - implement proper verification above
      throw new BadRequestException(
        'Google OAuth not fully configured. Please set up Google OAuth credentials in .env',
      );
    } catch (error) {
  ...
  async refreshToken(refreshToken: string): Promise<AuthResponseDto> {
    try {
      ...
      // For MVP, since refresh token is disabled, we'll find user by JWT payload
      const actualUser = await this.authRepository.findByEmail(payload.email);
      if (!actualUser) {
        throw new UnauthorizedException('User not found');
      }

      // Generate new tokens (refresh token rotation)
      const tokens = await this.generateTokens(actualUser.id, actualUser.email, actualUser.roles);

      // Save new refresh token and invalidate old one
      await this.authRepository.saveRefreshToken(actualUser.id, tokens.refreshToken);
